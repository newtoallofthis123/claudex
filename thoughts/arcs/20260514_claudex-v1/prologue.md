...
- Q: How should the parser respond to unknown / non-conversational events? → A: Convert them into compact `UnknownEvent` blocks (preserving `type` and a truncated raw blob) rather than crash. Structurally broken JSONL (a line that does not parse as JSON) is a hard failure that reports path + line number.
...
- Q: Launch process semantics? → A: `claude "<prompt>"` and `codex "<prompt>"` via `std::process::Command` with stdio inherited so the user lands directly in the new interactive session. The catch-up prompt is the positional argument verbatim. Missing executable surfaces `LaunchError::ExecutableNotFound` after the handoff file has been written; the command exits non-zero but the file path is still printed.
