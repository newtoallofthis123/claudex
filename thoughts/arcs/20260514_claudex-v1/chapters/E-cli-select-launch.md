...
- Launch failure (missing executable, non-zero exit before exec) prints the handoff path, prints a clear error, and exits non-zero. The written file is left in place.
- `claudex settings show` prints the parsed config plus the resolved effective values for handoff dir and per-agent roots. `path` prints the config file path. `edit` opens `$EDITOR` on the config file (creating it if missing). `set handoff_dir <path>`, `add-root`, `remove-root`, `reset-root` each mutate the TOML file in place using a load-modify-write round-trip and print the new effective config diff (or just "ok").
...
- **Implementation order:**
  ...
  8. Otherwise build the catch-up prompt from `launch::catch_up_prompt(&path)` and call `Launcher::launch(target, &prompt)`. On launch error, print `launch failed: <err>` to stderr and exit code 2. The handoff file remains.
...
- **Implementation:**
  ...
  - `show` → load the raw `Config`, then compute effective handoff dir + effective roots per agent, print both sections in plain text.
  - `edit` → ensure the file exists (create with `Config::default()` serialized to TOML if missing), then `Command::new(env!("EDITOR", "vi")).arg(path).status()`. Use `std::env::var("EDITOR").unwrap_or("vi")`.
  - `set handoff_dir <path>` → load, mutate `cfg.handoff_dir = Some(<path>)`, write.
  ...
  - Unknown `set <key>` values → error listing the supported keys (v1: only `handoff_dir`).
...
- **Cases:**
  1. `list_claude_lists_fixtures` — sets `CLAUDEX_CONFIG_FILE` (a tiny escape hatch we may add, or override via `--config` flag if simpler) or arranges `XDG_CONFIG_HOME` to point at a temp config that names the fixture root. Runs `claudex list claude` and asserts the fixture session ids appear, newest first.
  ...
  7. `settings_roundtrip` — `claudex settings add-root claude /tmp/foo` then `claudex settings show` lists `/tmp/foo`; `reset-root claude` removes it.
...
- **Config injection:** to point the tests at fixture roots, prefer giving the binary a `--config <path>` global flag — simpler than env wrangling. (This is a small addition to chapter A's CLI surface — chapter E may add it during integration.) If you do add `--config`, document it in `settings show` output too.
